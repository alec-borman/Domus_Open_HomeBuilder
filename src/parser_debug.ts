...
export function parse(input: string): { ast: Program, diagnostics: ParseDiagnostic[], treeNodeCount: number } {
  ...
  function extractFullReference(startIdx: number): string {
      let full = tokens[startIdx].value;
      let curr = startIdx + 1;
      while (curr < tokens.length) {
          let t = tokens[curr];
          if (t.type === 'Whitespace' || t.type === 'Comment') { curr++; continue; }
          if (t.value === '.' || t.value === '::') {
              let nextIdPos = curr + 1;
              while (nextIdPos < tokens.length && (tokens[nextIdPos].type === 'Whitespace' || tokens[nextIdPos].type === 'Comment')) nextIdPos++;
              if (nextIdPos < tokens.length && tokens[nextIdPos].type === 'Identifier') {
                  full += t.value + tokens[nextIdPos].value;
                  curr = nextIdPos + 1;
                  continue;
              }
          }
  ...
  function checkUnresolved(fullIdentifier: string, token: Token, obj?: any) {
                    ...
                    if (!isPropAccess) {
                        if (!isCurrentProperty || isRhs) {
                             if (!isCurrentProperty || nextPk?.value === '::') {
                                 checkUnresolved(extractFullReference(pos - 1), adv);
                             }
                        }
                    }
                }
